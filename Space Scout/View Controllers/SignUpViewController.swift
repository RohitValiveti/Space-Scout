    ...
    @objc func didTapSignUp(){
        ...
        if error != nil {
            ...
            Auth.auth().createUser(withEmail: email, password: pwd) { (response, error) in
                // Constants for user's username and uid
                ...
                if error != nil{
                    ...
                    // DocID = user's uid
                    // username = firstname+lastname lowercased
                    let db = Firestore.firestore()
                    db.collection("users").document(uid).setData(["firstname": firstname, "lastname": lastname, "username": username ]) { error in
                        if error != nil{
                            self.showError("There was an error saving user data.")
                        }
                    }

                    // Change to Home Screen, passing through user's uid
